        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 sm:gap-8 mb-12 sm:mb-16">
          ...
          >
            ...
            <h2 className="mt-2 text-[9vw] sm:text-4xl md:text-5xl lg:text-6xl font-black tracking-tight text-slate-900 dark:text-slate-50">
              Public Projects<span className="text-blue-600 dark:text-blue-400">.</span>
            </h2>
          </motion.div>

          <div className="relative z-10 flex flex-wrap items-center gap-1 bg-slate-100/80 dark:bg-slate-900/60 backdrop-blur-md p-1 rounded-2xl border border-slate-200/60 dark:border-slate-800/70 text-xs sm:text-[0.8rem]">
            {categories.map((cat) => (
              <button
                key={cat}
                  ...
                  <div className="relative h-full flex flex-col justify-between">
                    <div>
                      ...
                      <h3
                        className={`font-black text-slate-900 dark:text-slate-50 leading-tight group-hover:text-blue-600 dark:group-hover:text-blue-300 transition-colors ${
                          isFeatured ? 'text-lg sm:text-xxl' : 'text-lg sm:text-xl'
                        }`}
                      >
                        {project.title}
                      </h3>
                      <p
                        className={`text-slate-500 dark:text-slate-400 mt-3 font-medium leading-relaxed line-clamp-2 ${
                          isFeatured ? 'text-sm sm:text-sm' : 'text-xs sm:text-sm'
                        }`}
                      >
                        {project.description}
                      </p>
                    ...
                    <div className="flex flex-wrap gap-1.5 sm:gap-2 mt-4">
                      {project.tech.slice(0, isFeatured ? 6 : 3).map((t) => (
                        <span
                          key={t}
                          className="mb-4 px-2.5 py-1 bg-slate-100/80 dark:bg-slate-800/80 rounded-lg text-[10px] font-semibold text-slate-600 dark:text-slate-300 border border-slate-200/80 dark:border-slate-700/80"
                        >
                          {t}
                        </span>
                      ))}
